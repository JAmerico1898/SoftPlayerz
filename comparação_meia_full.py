...
#Plotar Segundo Gráfico - Radar de Percentis do Jogador na liga:
st.markdown("---")
st.markdown("<h3 style='text-align: center; color: blue; '>Percentis e Métricas de<br>Participação</h3>", unsafe_allow_html=True)
Participação_Meia_Charts_1 = Meia_Charts_1.copy()

metrics = Participação_Meia_Charts_1.iloc[:, np.r_[3, 72:74, 31, 74]].reset_index(drop=True)
## parameter names
...
Pressão_Linha_Alta_Meia_Charts = Meia_Charts_1.copy()

#Collecting data to plot
metrics = Pressão_Linha_Alta_Meia_Charts.iloc[:, np.r_[3, 70, 69, 82:84]].reset_index(drop=True)
## parameter names
params = list(metrics.columns[1:])
...
#Plotar Segundo Gráfico - Radar de Percentis do Jogador na liga:
st.markdown("---")
st.markdown("<h3 style='text-align: center; color: blue; '>Percentis e Métricas de<br>Impacto do Passe</h3>", unsafe_allow_html=True)

Impacto_Passe_Meia_Charts = Meia_Charts_1.copy()

#Collecting data to plot
metrics = Impacto_Passe_Meia_Charts.iloc[:, np.r_[3, 75:80]].reset_index(drop=True)
## parameter names
params = list(metrics.columns[1:])
...
Criação_Oportunidades_Meia_Charts = Meia_Charts_1.copy()

#Collecting data to plot
metrics = Criação_Oportunidades_Meia_Charts.iloc[:, np.r_[3, 86, 88:92]].reset_index(drop=True)
...
Drible_Meia_Charts = Meia_Charts_1.copy()

#Collecting data to plot
metrics = Drible_Meia_Charts.iloc[:, np.r_[3, 48, 54:57]].reset_index(drop=True)
...
Efetividade_Meia_Charts = Meia_Charts_1.copy()

#Collecting data to plot
metrics = Efetividade_Meia_Charts.iloc[:, np.r_[3, 57:64]].reset_index(drop=True)
## parameter names
params = list(metrics.columns[1:])
...
Ameaça_Ofensiva_Meia_Charts = Meia_Charts_1.copy()

#Collecting data to plot
metrics = Ameaça_Ofensiva_Meia_Charts.iloc[:, np.r_[3, 38:43]].reset_index(drop=True)
## parameter names
params = list(metrics.columns[1:])
